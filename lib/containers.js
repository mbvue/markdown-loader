...
//获取内容
function getContentToken(tokens, idx) {
        ...
        .use(container, 'demo', {
            render: function(tokens, idx) {
                ...
                if(tokens[idx].nesting === 1){
                    ...
                    let content = getContentToken(tokens, idx);

                    if(description.toLowerCase() === 'vue'){
                        return `<demo-block isComponent="${Buffer.from(content).toString('base64')}"><template v-slot:highlight>${highlight(parser, content, 'js')}</template></demo-block>`;
                    }else{
                        return `<demo-block><template v-slot:highlight>${highlight(parser, content, description)}</template><template v-slot:source>${content}</template></demo-block>`;
                    }
                }else{
                    return ``;
                }
